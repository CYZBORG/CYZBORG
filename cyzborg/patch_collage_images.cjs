...
const regex = /\{\/\* CULT CLASSIC SECTION \*\/\}[\s\S]*?(?=<\/section>\s*<\/>\);)/;

const newBlock = `{/* CULT CLASSIC SECTION */}
    <section className="py-6 md:py-12 bg-neutral-100 dark:bg-[#0a0a0a] relative border-t border-neutral-200 dark:border-neutral-900 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full flex flex-col lg:flex-row items-center lg:items-start gap-8 lg:gap-16">
        ...
        <div className="flex-1 text-center lg:text-left order-2 lg:order-1 relative z-10">
          <div className="inline-flex items-center gap-2 mb-4 justify-center lg:justify-start w-full">
            <span className="font-mono text-xs uppercase tracking-[0.3em] text-cyzborg-orange font-bold">
              FAN FAVORITE // HEAVY ROTATION
            </span>
          </div>
          
          <h3 style={{ fontFamily: '"Creepster", system-ui' }} className="text-6xl sm:text-7xl md:text-8xl text-cyzborg-orange mb-1 drop-shadow-lg tracking-wider">
            CULT CLASSIC
          </h3>
          ...
          <div className="font-sans text-neutral-600 dark:text-neutral-400 text-sm md:text-base lg:text-lg xl:text-xl lg:leading-relaxed tracking-wide max-w-lg mx-auto lg:mx-0 mb-6 drop-shadow-sm">
            <p>
        ...
        <div className="relative w-full max-w-md lg:max-w-[460px] xl:max-w-[500px] 2xl:max-w-[600px] mx-auto aspect-[5/4] sm:aspect-[10/9] lg:aspect-square order-1 lg:order-2 lg:mt-0 min-w-0">
          ...
          <div className="absolute top-0 left-[10%] sm:left-[5%] w-[80%] sm:w-[90%] aspect-square border border-neutral-500 shadow-[0_20px_50px_rgba(0,0,0,0.7)] bg-white dark:bg-black overflow-hidden group/album z-20 transition-all duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] hover:scale-[1.02] hover:-translate-y-2">
