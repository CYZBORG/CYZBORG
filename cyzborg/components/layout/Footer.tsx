  ...
  return (
    <footer className="bg-cyzborg-black py-12 border-t border-neutral-900 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center relative z-10">
        ...
        <div 
          className="flex items-center justify-center md:justify-start gap-1.5 order-1 cursor-pointer"
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
        >
           <img 
            src="https://helmet-with-logo.netlify.app/cyzborg-leds-logo.svg" 
            alt="CYZBORG" 
            className="peer w-auto h-[30px] md:h-9 opacity-80 hover:opacity-100 hover:scale-x-[1.08] hover:scale-y-[1.02] transition-all duration-700 ease-out origin-left"
           />
           <img 
            src="https://helmet-with-logo.netlify.app/new%20flat%20logo.png" 
            alt="" 
            className="w-auto h-[42px] md:h-[54px] opacity-80 hover:opacity-100 hover:scale-110 hover:drop-shadow-[0_0_2px_rgba(0,102,204,1)] peer-hover:translate-x-3 transition-all duration-700"
           />
        </div>
        ...
        <div className="text-neutral-600 text-[10px] md:text-xs font-sans uppercase tracking-[0.2em] text-center order-3 md:order-2 flex flex-col gap-1.5">
          <span>&copy; 2026 CYZBORG™. All Rights Reserved.</span>
